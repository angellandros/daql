...
def divide_the_data(file_name, k=5, dump=True, user_count=None):
    ...
    for i in range(k):
        # creating k maps of lists in the folds
        folds.append(defaultdict(list))
    user_index = 0
            ...
            with open('ratings_fold_%d.csv' % fold_index, 'w') as f:
                for user, ratings in folds[fold_index].items():
                    for item, rate, timestamp in ratings:
                        # timestamp contains newline character
                        f.write('%s,%s,%s,%s' % (user, item, rate, timestamp))
...
def test_once(train_sets, loaded_test_set, k=10):
    ...
    for train_set in train_sets:
        ...
        logging.debug('engine size: %d' % sum(len(val)
                                              for _, val in engine.ratingsIndex.items()))
    ...
    for user, ratings in loaded_test_set.items():
        ...
        pred_dict = {item: rate for item,
                     rate in pred_list if type(rate) == float}
        ...
        logging.debug('number of correct recommendations for user %s: %d/%d' %
                      (user, len(common_keys), len(ratings)))
        test_results['rmse'].append(metric_rmse(pred_dict, echt_dict))
        test_results['mae'].append(metric_rmse(pred_dict, echt_dict))
        test_results['precision@k'].append(
            metric_precision_at_k(pred_list, echt_list, k))
        test_results['recall@k'].append(
            metric_recall_at_k(pred_list, echt_list, k))
...
def test_5_times(folds, names, k=10):
    test_results = defaultdict(list)
    for i in range(5):
        _, mean_result = test_once([names[j]
                                    for j in range(5) if i != j], folds[i], k=k)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data-set',
                        help='the csv file of ratings', default='ratings.csv')
